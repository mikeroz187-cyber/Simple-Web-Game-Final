...
--- | --- | --- | --- | --- | ---
Rival studios / competitive pressure | `/js/systems/competition.js` (new, TBD) or `/js/systems/progression.js` (extended) | `rivals`, `market`, `player`, `reputation` | `rivals`, `market`, `player.reputation` (if modified) | `CONFIG.rivals.*`, `CONFIG.market.*` | TBD (scope allows new system)
Structured high-impact events (Act 3) | `/js/systems/story.js` (extended) or `/js/systems/events.js` (new, TBD) | `story.act3`, `player`, `events` (if present) | `story.act3`, `events` (if present) | `CONFIG.story.act3.*`, `CONFIG.events.act3.*` | Expanded (story) or TBD (new events system)
Reputation branches (studio identity paths) | `/js/systems/progression.js` (extended) | `reputation`, `player.reputation`, `milestones` | `reputation`, `milestones` (if used) | `CONFIG.reputation.branches.*`, `CONFIG.milestones.*` | Expanded
Act 3 story arc progression | `/js/systems/story.js` (extended) | `story.act3`, `player.day`, `milestones` (if gating) | `story.act3` | `CONFIG.story.act3.*` | Expanded
Optional automation (late-game QoL) | `/js/systems/automation.js` (new, TBD) | `automation`, `schedule`, `player`, `roster` | `automation`, `schedule` | `CONFIG.automation.*` | TBD (new)
Content performance variance | `/js/systems/content.js` (new, TBD) and/or `/js/systems/analytics.js` (extended) | `content.entries`, `content.variance`, `analyticsHistory` | `content.entries`, `content.variance`, `analyticsHistory` | `CONFIG.content.variance.*`, `CONFIG.analytics.*` | TBD (new/extended)
Advanced scheduling | `/js/systems/booking.js` (extended) and/or `/js/systems/time.js` (new, TBD) | `schedule`, `roster`, `player` | `schedule`, `player.day` (if queued resolution) | `CONFIG.schedule.*`, `CONFIG.booking.*` | Expanded or TBD (new)
Legacy / recognition milestones | `/js/systems/progression.js` (extended) or `/js/systems/milestones.js` (new, TBD) | `legacyMilestones`, `player`, `milestones` | `legacyMilestones` | `CONFIG.legacyMilestones.*` | Expanded or TBD (new)
...
### /js/systems/competition.js (Act 3 additions, TBD if new)
- Purpose (Act 3)
...
### /js/systems/story.js (Act 3 additions)
- Purpose (Act 3)
...
### /js/systems/progression.js (Act 3 additions)
- Purpose (Act 3)
...
### /js/systems/automation.js (Act 3 additions, TBD if new)
- Purpose (Act 3)
...
### /js/systems/analytics.js (Act 3 additions, extended)
- Purpose (Act 3)
...
### /js/systems/content.js (Act 3 additions, TBD if new)
- Purpose (Act 3)
...
### /js/systems/booking.js (Act 3 additions)
- Purpose (Act 3)
...
### /js/systems/milestones.js (Act 3 additions, TBD if new)
- Purpose (Act 3)
