...
- Existing screens expanded: Hub, Booking, Content, Analytics, Roster (Social TBD), Shop (no Act 3 changes unless scope later adds items).
...
| screen-booking | Booking | MVP + Act2 + Act3 | Plan and confirm a shoot + advanced scheduling (Act 3) | Confirm Shoot (MVP); Queue Booking (Act 3, TBD) |
...
- **Panel Name:** Rival Studios & Market Status
  ...
  - Displays: rival standings summary (TBD), active market shifts (TBD)
  - Actions (buttons/controls):
    - Label: “Evaluate Rivals” (TBD)
    - Enabled/disabled rules: enabled only if rivals config exists (TBD)
...
- **Panel Name:** Act 3 Story & Events
  ...
  - Displays: current Act 3 story status/flags (TBD fields)
  - Actions (buttons/controls):
    - Label: “Resolve Event” (TBD)
    - Enabled/disabled rules: enabled only when a pending event is present (TBD)
...
- **Panel Name:** Reputation Branch (Studio Identity)
  ...
  - Displays: selected branch ID/name (TBD), branch progress (TBD)
  - Actions (buttons/controls):
    - Label: “Select Branch” (TBD)
    - Enabled/disabled rules: enabled only if no branch selected and eligibility met (TBD)
...
- **Panel Name:** Legacy Milestones
  ...
  - Displays: milestone list with status (TBD record shape)
  - Actions (buttons/controls):
    - Label: “Claim Reward” (TBD, per milestone)
    - Enabled/disabled rules: enabled only if milestone eligible (TBD)
...
- **Panel Name:** Automation Status (Optional)
  ...
  - Displays: automation enabled/disabled summary (TBD fields)
  - Actions (buttons/controls):
    - Label: “Configure Automation” (TBD)
    - Enabled/disabled rules: enabled only if automation feature is enabled in config (TBD)
...
- **Panel Name:** Schedule Summary (Advanced Scheduling)
  ...
  - Displays: queued bookings count and next scheduled slot (TBD)
  - Actions (buttons/controls):
    - Label: “Resolve Schedule” (TBD)
    - Enabled/disabled rules: enabled only if schedule queue has entries (TBD)
...
- **Panel Name:** Advanced Scheduling
  - Reads from gameState: `schedule`
  - Displays: queue slots, current queue length, schedule rules (TBD)
  - Actions (buttons/controls):
    - Label: “Queue Booking”
    - Enabled/disabled rules: enabled only if scheduling enabled and queue not full (TBD)
...
- **Panel Name:** Variance Notes
  ...
  - Displays: variance applied to latest content entry (TBD fields)
  - Actions (buttons/controls):
    - None (read-only)
...
- **Panel Name:** Variance Summary
  ...
  - Displays: variance rollups and bounds summary (TBD)
  - Actions (buttons/controls):
    - None (read-only)
...
- **Panel Name:** Rival / Market Impact Notes
  - Reads from gameState: `rivals`, `market`
  - Displays: active rival pressure and market modifiers (TBD)
  - Actions (buttons/controls):
    - None (read-only)
...
- **Panel Name:** Act 3 Events Impact
  ...
  - Displays: last event impact summary (TBD)
  - Actions (buttons/controls):
    - None (read-only)
...
- **Panel Name:** Reputation Branch Effects
  ...
  - Displays: branch effects summary (TBD) and any roster impacts (TBD)
  - Actions (buttons/controls):
    - None (read-only)
  - Empty/edge states: “No branch selected.”
...
#### Panels / Sections
- **Panel Name:** TBD
  - Reads from gameState: TBD
  - Displays: TBD
  - Actions (buttons/controls): TBD
  - Empty/edge states: TBD
...
- **How progress is shown:** milestone list with status (locked/in-progress/complete) — exact fields TBD.
