...
Repo layout is authoritative here: JS lives in `/src`, and CSS lives in `/styles.css`.
...
| `styles.css` | Global theme variables, layout, screen visibility helpers, button/card/modal styles | Framework-like utility explosion, mobile/responsive breakpoints |
| `src/config.js` | Single `CONFIG` object with **all tunables** (starting cash, costs, thresholds, timers, multipliers) | Dynamic runtime state, DOM work |
| `src/state.js` | `gameState` schema + `newGameState()` factory + default values | DOM, rendering, save I/O, random gameplay calculations |
| `src/save.js` | Save/load/reset/export/import helpers | Rendering, business logic, direct DOM manipulation beyond file input handling |
| `src/main.js` | App bootstrap (init), wiring modules, starting screen, initial render | Heavy gameplay rules, hard-coded config values |
| `src/ui/router.js` | Show/hide screens, enforce “exactly one active screen” | Gameplay rules, state calculations |
| `src/ui/render.js` | Render functions per screen using `gameState` as input | Deep business logic; state mutations should go through systems |
| `src/ui/events.js` | Event listeners that call systems + then re-render | State schema definitions or config definitions |
| `src/ui/components.js` | Tiny DOM builder helpers for repeated UI pieces (cards, rows, toasts) | App routing, save logic, config |
| `src/systems/*.js` | “Pure-ish” gameplay logic (mutates `gameState` in controlled ways), returns result objects | DOM access, direct `localStorage`, direct screen navigation |
| `src/utils/*.js` | Small pure helpers (query selectors, formatting money, seeded RNG) | App state ownership, business rules |
...
6) src/utils/rng.js
7) src/systems/economy.js
8) src/systems/performers.js
9) src/systems/booking.js
10) src/systems/social.js
11) src/systems/progression.js
12) src/systems/story.js
13) src/ui/components.js
14) src/ui/router.js
15) src/ui/render.js
16) src/ui/events.js
17) src/main.js
